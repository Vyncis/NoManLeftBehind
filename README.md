...
See that in `src`, we have `vyncis.nomanleftbehind` as the package (which matches the folder path `src/vyncis/modname`).
...
1. [ ] The first step is to change the package of our code. This is an organization thing; if there are two Java files
   ...
   will be loaded.
    1. The easy way to change this is to open up `TemplateModPlugin.java`, click on the first line (`package...`),
       ...
       .
    2. When done, you shouldn't have any code in `vyncis.nomanleftbehind`. If that still exists, it can be safely deleted.
    3. 
